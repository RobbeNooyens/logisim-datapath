...
def int2bin(s, width):  # convert a signed integer to a two's complement bin notation
    if (int(s) > 2 ** (int(width) - 1) - 1 or int(s) < -(2 ** (int(width) - 1))):
        raise ValueError(
            "Number %s exceeds supported range of [%d, %d]" % (s, -(2 ** (int(width) - 1)), 2 ** (int(width) - 1) - 1))
    else:
...
def label2bin(s, width, symboltable,
              count_from=None):  # convert a label to a binary jump value (by looking in the symboltable), if count_from (int) than jump relative to that memory address
...
def writeregister():
    return "r(?:" + "|".join(
        [str(i) for i in set(range(0, REGISTER_ADDRESS_SIZE ** 2)) - set(READONLY_REGISTERS)]) + ")"
        ...
        if not (line == "\n" or line == os.linesep or skipline.search(line)):
            if mode == "idle":
                ...
                if datamem.search(line):
                    if programlinenr == 0:
                        print(
                            "warning: line %d: wanted to start a data section, but expects instructions first" % linenr)
                    programlinenr += 1
                    debugtraces[curtestfile][programlinenr] = {"linenr": linenr, "line": "STOP", "bin": "0" * op_width,
                                                               "hex": ubin2hex("0" * op_width, op_width),
                                                               "name": "STOP", "checks": dict()}
                    # print "change to datamem"
                    mode = "datamem"
                elif checkmem.search(line):
                    if programlinenr == 0:
                        print(
                            "warning: line %d: wanted to start a check section, but expects instructions first" % linenr)
                    ...
                    for opname, oppattern, opdescription, opparser in operations:
                        ...
                        if m:
                            try:
                                ...
                                debugtraces[curtestfile][programlinenr] = {"linenr": linenr, "line": line.strip(),
                                                                           "bin": binary,
                                                                           "hex": ubin2hex(binary, op_width),
                                                                           "name": opname, "checks": dict()}
                    ...
                    if not matched:
                        print("line %d not recognized: '%s', but should be of the form: %s" % (
                        linenr, line.strip(), str(matcheddesc) if matcheddesc else ""))
                        del debugtraces[curtestfile]
                        mode = "idle"
            elif mode == "datamem":
                if end.search(line):
                    ...
                    for immpattern in [valueline, bvalueline]:
                        ...
                        if m:
                            ...
                            debugtraces[curtestfile][programlinenr] = {"linenr": linenr, "line": line.strip(),
                                                                       "bin": binary,
                                                                       "hex": ubin2hex(binary, data_width),
                                                                       "name": "imm", "checks": dict()}
                            matched = "imm"
                            break
                    if not matched:
                        print("line %d not recognized: '%s', but should be %d-bit data" % (
                        linenr, line.strip(), data_width))
                        del debugtraces[curtestfile]
                        mode = "idle"
                ...
                if end.search(line):
                    ...
                    if fulldatapath:
                        print(
                            "warning: when checking the full datapath, only checks at the end of the program will be made")
                    # print "change to loadmem"
                    mode = "loadmem"
                    ...
                    if m:
                        ...
                        while lastop["name"] in ["imm", "STOP"]:
                            ...
                            i -= 1
                        lastop["checks"][
                            0 if m.group("reg") == "pc:" else 1 + int(m.group("reg").rstrip(":").lstrip("r"))] = binval
                    else:
                        print("line %d not recognized: '%s', but should be of the form: reg: value" % (
                        linenr, line.strip()))
                        del debugtraces[curtestfile]
                        mode = "idle"
        line = f.readline()
...
def Datapathparser(reportfile, debugtrace, width, nr_of_tests, fulldatapath=True):
        ...
        for item in items:
            ...
            if 'E' in item or 'x' in item:
                if not fulldatapath:
                    ...
                    print("Warning: %s has value %s, at line %d: %s" % (
                    regname, item, curtrace["linenr"], curtrace["line"]))
                else:
    ...
    if not fulldatapath:
        ...
        if len(lines) == len(instructionkeys) + 1:
            print("Warning: report file %s should have %d lines but has %d lines" % (
            reportfile, len(instructionkeys) + 2, len(lines)))
        assert len(lines) == len(instructionkeys) + 1 or len(lines) == len(
            instructionkeys) + 2, "Report file %s should have %d lines but has %d lines (this is due to a Logisim error. Possible cause: you used multiple clock instances in your (sub)circuits, whereas you should only use a single clock instance. -- Contact the author of this Python program for help)" % (
        reportfile, len(instructionkeys) + 2, len(lines))
    ...
    for i in range(1, len(instructionkeys) + 1):  # start counting from 1
        ...
        if (len(checks) > 0) and (not fulldatapath or i == len(
                instructionkeys)):  # only check last instruction in case of a full datapath, otherwise we're in trouble in case of a loop
            if not fulldatapath:
                line = lines[i]
            else:
                line = lines[
                    -2]  # the second to last, because the last one is the one we already halted (pc will be increased)
                ...
                if line.split("\t")[1:] != lines[-1].split("\t")[1:]:  # register values did change last step
                    line = lines[-1]
                elif int(line.split("\t")[0].replace(" ", "").strip(), 2) + 1 != int(
                        lines[-1].split("\t")[0].replace(" ", "").strip(), 2):  # last step was a branch
                    line = lines[-1]
            items = [item.replace(" ", "").strip() for item in line.split("\t")]
            if fulldatapath:
            ...
            for reg in checks.keys():
                ...
                if 'E' in value or 'x' in value:
                    errors += 1
                    print("Error: %s has value %s, at line %d: %s" % (
                    regname, value, curtrace["linenr"], curtrace["line"]))
                elif oracle != value:
                    if int(value, 2) + 1 == int(oracle, 2) and regname == "pc" and not line == lines[
                        -1]:  # it may still be true that we actually need to look at the last entry... just throw warning
                        print("Warning: pc may not have been increased yet (expected %s, got %s)" % (oracle, value))
                    else:
                        failures += 1
                        print("Failure: %s must be %s, but is %s, at line %d: %s" % (
                        regname, oracle, value, curtrace["linenr"], curtrace["line"]))
...
def ALUtestcompiler(textfile, testfile, width, nr_of_tests):
    ...
    for line in f.readlines():
            ...
            try:
                if not (len(param) == width):  # if its not a binary string...
                    if (int(param) > 2 ** (width - 1) - 1 or int(param) < -(2 ** (width - 1))):
                        print("Number exceeds supported ALU range of [%d, %d] on line %d: %s" % (
                        -(2 ** (width - 1)), 2 ** (width - 1) - 1, linenr, line))
                        return False
                else:
                    try:
        ...
        for param in params[4:]:
            try:
                if not int(param) in [0, 1]:
                    print(
                        "Parameter denoting that there should/shouldn't be an error must be 1 or 0, but is %s in line %s" % (
                        param, line))
                    return False
            except Exception:
                print(
        ...
        if linenr == nr_of_tests:
            print(
                "MAXIMUM NUMBER OF TESTS (%d) REACHED! IGNORING FURTHER TESTS. (You can split up your tests over two test files.)" % nr_of_tests)
            break
    ...
    while linenr <= len(debugtrace) - 1:
        ...
        if 'E' in line or 'x' in line:
            ...
            if debugtrace[linenr][0].split()[0] in zerary_ops:
                op = "Operation %s ('%s'), result is %s, error code is %s" % (
                cells[OP], debugtrace[linenr][0].split()[0], cells[RESULT], cells[ERROR])
            elif debugtrace[linenr][0].split()[0] in unary_ops:
                op = "Operation %s ('%s') with operand %s (%s), result is %s, error code is %s" % (
                cells[OP], debugtrace[linenr][0].split()[0], cells[A], debugtrace[linenr][0].split()[1], cells[RESULT],
                cells[ERROR])
            elif debugtrace[linenr][0].split()[0] in binary_ops:
                op = "Operation %s ('%s') with operands %s (%s) and %s (%s), result is %s, error code is %s" % (
                cells[OP], debugtrace[linenr][0].split()[0], cells[A], debugtrace[linenr][0].split()[1], cells[B],
                debugtrace[linenr][0].split()[2], cells[RESULT], cells[ERROR])
            else:
            ...
            # print debugtrace[linenr][1],
            if debugtrace[linenr][0].split()[0] in zerary_ops:
                op = "Operation %s ('%s'), result is %s (%s)" % (
                cells[OP], debugtrace[linenr][0].split()[0], cells[RESULT], twoscompl_bin2dec(cells[RESULT], width))
            elif debugtrace[linenr][0].split()[0] in unary_ops:
                op = "Operation %s ('%s') with operand %s (%s), result is %s (%s)" % (
                cells[OP], debugtrace[linenr][0].split()[0], cells[A], debugtrace[linenr][0].split()[1], cells[RESULT],
                twoscompl_bin2dec(cells[RESULT], width))
            elif debugtrace[linenr][0].split()[0] in binary_ops:
                op = "Operation %s ('%s') with operands %s (%s) and %s (%s), result is %s (%s)" % (
                cells[OP], debugtrace[linenr][0].split()[0], cells[A], debugtrace[linenr][0].split()[1], cells[B],
                debugtrace[linenr][0].split()[2], cells[RESULT], twoscompl_bin2dec(cells[RESULT], width))
            else:
                op = "Unknown operation..."
            ...
            if int(cells[ERR_ORACLE]) == 0 and cells[RESULT] != cells[
                ORACLE]:  # results are only compared when no error is raised (int(cells[ERR_ORACLE]) == 0)
                print("%s%s" % (str(debugtrace[linenr][0]), op))
                print("Expected %s (%d) as result, but got %s (%d)" % (
                cells[ORACLE], twoscompl_bin2dec(cells[ORACLE]), cells[RESULT], twoscompl_bin2dec(cells[RESULT])))
            print("")
...
def Test(textfile, circfile, compiler, parser, logisim):
    ...
    def remove_absolute_paths_from_circ_file(filename):
        ...
        for line in f.readlines():
            ...
            if m:
                ...
                reffiles.append(
                    reffile)  # DO NOT use os.path.basename because it uses the pathseparator of your current os
                if path != reffile:
    ...
    for testfile in testfiles:
        ...
        if TIMEOUT <= 0:
            import subprocess
            p = subprocess.Popen(["java", "-jar", logisim, circfile, "-tty", "table", "-load", testfile], stdout=f,
                                 stderr=f2)
            p.wait()
        else:
            ...
            def target():
                global process
                process = subprocess.Popen(["java", "-jar", logisim, circfile, "-tty", "table", "-load", testfile],
                                           stdout=f, stderr=f2)
                process.communicate()
...
def main():
    if __name__ == "__main__":
        ...
        parser.add_argument("-s", "--simpledatapath", action="store_true", dest="sdp",
                            help="test simple datapath (only r-type and sb/lb)")
        parser.add_argument("-f", "--fulldatapath", action="store_true", dest="fdp",
                            help="test full datapath (all instructions)")
        ...
        parser.add_argument("--logisimpath", type=str, dest="path", default="logisim-generic-2.7.1.jar",
                            help="sets logisim jar to <PATH>")
        parser.add_argument("--timeout", type=int, dest="timeout", default="20",
                            help="sets the timeout in seconds for infinite loops to TIMEOUT (0 means no timeout) (default: 20s)")
...
DP_OPERATIONS = [
    ('zero', pattern((('op', 'zero'), ('rd', register())), can_have_label=True), "zero rd            -->  $rd := 0",
     lambda m, params: '0000' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + '0000' + '0000'),
    ('not', pattern((('op', 'not'), ('rd', register()), ('rs', register())), can_have_label=True),
     "not rd rs          -->  $rd := ! $rs",
     lambda m, params: '0001' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + '0001'),
    ('inv', pattern((('op', 'inv'), ('rd', register()), ('rs', register())), can_have_label=True),
     "inv rd rs          -->  $rd := - $rs",
     lambda m, params: '0001' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + '1010'),
    ('sll', pattern((('op', 'sll'), ('rd', register()), ('rs', register())), can_have_label=True),
     "sll rd rs          -->  $rd := $rs << 2",
     lambda m, params: '0001' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + '1011'),
    ('srl', pattern((('op', 'srl'), ('rd', register()), ('rs', register())), can_have_label=True),
     "srl rd rs          -->  $rd := $rs >> 2",
     lambda m, params: '0001' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + '1100'),
    ('sla', pattern((('op', 'sla'), ('rd', register()), ('rs', register())), can_have_label=True),
     "sla rd rs          -->  $rd := $rs * 2",
     lambda m, params: '0001' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + '1101'),
    ('sra', pattern((('op', 'sra'), ('rd', register()), ('rs', register())), can_have_label=True),
     "sra rd rs          -->  $rd := $rs / 2",
     lambda m, params: '0001' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + '1110'),
    ('cp', pattern((('op', 'cp'), ('rd', register()), ('rs', register())), can_have_label=True),
     "cp rd rs           -->  $rd := $rs",
     lambda m, params: '0001' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + '1111'),
    ('and', pattern((('op', 'and'), ('rd', register()), ('rs', register()), ('rt', register())), can_have_label=True),
     "and rd rs rt       -->  $rd := $rs & $rt",
     lambda m, params: '0010' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + str(reg2bin(m.group('rt'), REGISTER_ADDRESS_SIZE))),
    ('or', pattern((('op', 'or'), ('rd', register()), ('rs', register()), ('rt', register())), can_have_label=True),
     "or rd rs rt        -->  $rd := $rs | $rt",
     lambda m, params: '0011' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + str(reg2bin(m.group('rt'), REGISTER_ADDRESS_SIZE))),
    ('add', pattern((('op', 'add'), ('rd', register()), ('rs', register()), ('rt', register())), can_have_label=True),
     "add rd rs rt       -->  $rd := $rs + $rt",
     lambda m, params: '0100' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + str(reg2bin(m.group('rt'), REGISTER_ADDRESS_SIZE))),
    ('sub', pattern((('op', 'sub'), ('rd', register()), ('rs', register()), ('rt', register())), can_have_label=True),
     "sub rd rs rt       -->  $rd := $rs - $rt",
     lambda m, params: '0101' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + str(reg2bin(m.group('rt'), REGISTER_ADDRESS_SIZE))),
    ('lt', pattern((('op', 'lt'), ('rd', register()), ('rs', register()), ('rt', register())), can_have_label=True),
     "lt rd rs rt        -->  $rs < $rt ? $rd := 1 : $rd := 0",
     lambda m, params: '0110' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + str(reg2bin(m.group('rt'), REGISTER_ADDRESS_SIZE))),
    ('gt', pattern((('op', 'gt'), ('rd', register()), ('rs', register()), ('rt', register())), can_have_label=True),
     "gt rd rs rt        -->  $rs > $rt ? $rd := 1 : $rd := 0",
     lambda m, params: '0111' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + str(reg2bin(m.group('rt'), REGISTER_ADDRESS_SIZE))),
    ('eq', pattern((('op', 'eq'), ('rd', register()), ('rs', register()), ('rt', register())), can_have_label=True),
     "eq rd rs rt        -->  $rs = $rt ? $rd := 1 : $rd := 0",
     lambda m, params: '1000' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + str(reg2bin(m.group('rt'), REGISTER_ADDRESS_SIZE))),
    ('neq', pattern((('op', 'neq'), ('rd', register()), ('rs', register()), ('rt', register())), can_have_label=True),
     "neq rd rs rt       -->  $rs != $rt ? $rd := 1 : $rd := 0",
     lambda m, params: '1001' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + str(reg2bin(m.group('rt'), REGISTER_ADDRESS_SIZE))),
    ('lw',
     pattern((('op', 'lw'), ('rd', register()), ('rs', register()), ('imm', integerpattern())), can_have_label=True),
     "lw rd rs uimm      -->  $rd := MEM[$rs + imm]",
     lambda m, params: '1010' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + str(int2bin(m.group('imm'), 4))),
    ('sw',
     pattern((('op', 'sw'), ('rd', register()), ('rs', register()), ('imm', integerpattern())), can_have_label=True),
     "sw rd rs uimm      -->  MEM[$rs + imm] := $rd",
     lambda m, params: '1011' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         reg2bin(m.group('rs'), REGISTER_ADDRESS_SIZE)) + str(int2bin(m.group('imm'), 4)))
]
...
# (name, parse_pattern, help_string, compile_function)
FULL_DP_OPERATIONS = [
    ('ori', pattern((('op', 'ori'), ('rd', register()), ('imm', binarypattern(8))), can_have_label=True),
     "ori rs imm     -->  rd := rd | imm",
     lambda m, params: '1100' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + m.group('imm')),
    ('ori', pattern((('op', 'ori'), ('rd', register()), ('imm', integerpattern())), can_have_label=True),
     "ori rs imm     -->  rd := rd | imm",
     lambda m, params: '1100' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(int2bin(m.group('imm'), 8))),
    ('lui', pattern((('op', 'lui'), ('rd', register()), ('imm', binarypattern(8))), can_have_label=True),
     "lui rs imm     -->  rd := imm << 6",
     lambda m, params: '1101' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + m.group('imm')),
    ('lui', pattern((('op', 'lui'), ('rd', register()), ('imm', integerpattern())), can_have_label=True),
     "lui rs imm     -->  rd := imm << 6",
     lambda m, params: '1101' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(int2bin(m.group('imm'), 6))),
    ...
    ('brnz', pattern((('op', 'brnz'), ('rd', register()), ('imm', integerpattern())), can_have_label=True),
     "brnz rd imm     -->  rd!=0 ? pc := pc+1+imm",
     lambda m, params: '1110' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(int2bin(m.group('imm'), 8))),
    ('brnz', pattern((('op', 'brnz'), ('rd', register()), ('to', labelpattern())), can_have_label=True),
     "brnz rd imm     -->  rd!=0 ? pc := pc+1+imm",
     lambda m, params: '1110' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         label2bin(m.group('to'), 8, params[1], params[0] - 1))),
    ('j', pattern((('op', 'j'), ('addr', binarypattern(8))), can_have_label=True), "j addr          -->  pc := addr",
     lambda m, params: '1111' + m.group('addr') + '0000'),
    ('j', pattern((('op', 'j'), ('addr', uintegerpattern())), can_have_label=True), "j addr          -->  pc := addr",
     lambda m, params: '1111' + str(int2bin(m.group('addr'), 8)) + '0000'),
    ('j', pattern((('op', 'j'), ('to', labelpattern())), can_have_label=True), "j addr          -->  pc := addr",
     lambda m, params: '1111' + str(label2bin(m.group('to'), 8, params[1])) + '0100'),
    ('jr', pattern((('op', 'jr'), ('rd', register()), ('imm', binarypattern(4))), can_have_label=True),
     "jr rd imm       -->  pc := rd+imm",
     lambda m, params: '1111' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + m.group('imm') + '0100'),
    ('jr', pattern((('op', 'jr'), ('rd', register()), ('imm', integerpattern())), can_have_label=True),
     "jr rd imm       -->  pc := rd+imm",
     lambda m, params: '1111' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         int2bin(m.group('imm'), 4)) + '0100'),
    ('jr', pattern((('op', 'jr'), ('rd', register()), ('to', labelpattern())), can_have_label=True),
     "jr rd imm       -->  pc := rd+imm",
     lambda m, params: '1111' + str(reg2bin(m.group('rd'), REGISTER_ADDRESS_SIZE)) + str(
         label2bin(m.group('to'), 4, params[1])) + '0100'),
    ('jal', pattern((('op', 'jal'), ('addr', binarypattern(8))), can_have_label=True),
     "jal addr        -->  r7 := pc+1 ; pc := addr", lambda m, params: '1111' + m.group('addr') + '1111'),
    ('jal', pattern((('op', 'jal'), ('addr', uintegerpattern())), can_have_label=True),
     "jal addr        -->  r7 := pc+1 ; pc := addr",
     lambda m, params: '1111' + str(int2bin(m.group('addr'), 8)) + '1111'),
    ('jal', pattern((('op', 'jal'), ('to', labelpattern())), can_have_label=True),
     "jal addr        -->  r7 := pc+1 ; pc := addr",
     lambda m, params: '1111' + str(label2bin(m.group('to'), 8, params[1])) + '1111'),
]
